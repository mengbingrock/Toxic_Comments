...
class Preprocessor:
    ...
    def _parse_trained_data(self, orig_data):
        ...
        labels = orig_data[self.classes]
        train_data = remove_punctuation(text)
        return train_data, labels

    def _parse_test_data(self, test_data):
        self.logger.info("Parsing test dataset...")
        text = remove_punctuation(test_data[self.config['input_text']])
        ids = test_data.id
        return text, ids
    ...
    def nn_vecterization(self, train_x, validate_x, test_data):
        ...
        self.logger.info("Done. Got {} words".format(len(self.word2ind)))
